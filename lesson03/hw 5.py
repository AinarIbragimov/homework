...
# Эту задачу не успел до конца решить. Поджимала время до которого нужно было отправить задания.
def int_func(*args):
    numbers = int(input('Введите строку чисел, разделённых пробелом '))
    i = 0
    i += 1

    sum_numres = sum(numbers)
    return sum_numres
